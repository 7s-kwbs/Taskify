...
class MytaskScreen extends StatelessWidget{
  ...
  Widget build(BuildContext context){
    return Scaffold(
      ...
      body: Column(
        children: [
          DashboardHeader(title: "My tasks", isDashboard: false,),
          Expanded(
            child: Container(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  SizedBox(height: 5,),
                  ...todayTasks.map((task)=> _TaskCard(title: task.title, date: task.date, label: task.labels)),
                  SizedBox(height: 12,),
                  _SectionHeader(title: "Tommorow"),
                  SizedBox(height: 5,),
                 ...tomorrowTasks.map((task)=> _TaskCard(title: task.title, date: task.date, label: task.labels)),
                   SizedBox(height: 12,),
                  _SectionHeader(title: "This week"),
                  SizedBox(height: 5,),
                  ...thisWeekTasks.map((task)=> _TaskCard(title: task.title, date: task.date, label: task.labels))
                ],
              ),
            )
          ),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Row(
            children: label.map((label)=>_LabelChip(text: label.text,color: label.color,)).toList(),
          )
        ],
      ),
    );
